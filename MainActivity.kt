...
class MainActivity : AppCompatActivity() {
    ...
    override fun onStart() {
        ...
        val lemonTree = findViewById<ImageView>(R.id.limoeiro)

        var state = "tree"
        lemonTree.setOnClickListener { this
            lemonTree.setImageResource(R.drawable.lemon_squeeze)
            if(state == "tree"){
                state = "lemon"
            ...
            if(state == "lemon"){
                if(this.generateNumber()){
                    lemonTree.setImageResource(R.drawable.lemon_drink)
                    state = "full"
                    return@setOnClickListener
                } else {
            ...
            if(state == "full"){
                lemonTree.setImageResource(R.drawable.lemon_restart)
                Toast.makeText(this, "Clique para beber", Toast.LENGTH_SHORT).show()
                state = "empty"
                return@setOnClickListener
            }
            if(state == "empty"){
                lemonTree.setImageResource(R.drawable.lemon_tree)
                state = "tree"
                return@setOnClickListener
            }
        }
    ...
    private fun generateNumber(): Boolean {
        var random = Math.floor(Math.random() * 6 ).toString()
        var n = random.slice(0..0).toInt()
        return n == 3
    }
